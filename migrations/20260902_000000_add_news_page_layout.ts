...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ...
    ALTER TABLE "payload_locked_documents_rels"
      ADD COLUMN IF NOT EXISTS "news_page_layout_id" integer;

    DO $$ BEGIN
      ALTER TABLE "payload_locked_documents_rels"
        ADD CONSTRAINT "payload_locked_documents_rels_news_page_layout_fk"
        FOREIGN KEY ("news_page_layout_id") REFERENCES "public"."news_page_layout"("id")
        ON DELETE CASCADE;
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
    DROP TABLE IF EXISTS "news_page_layout";
  `)
}
