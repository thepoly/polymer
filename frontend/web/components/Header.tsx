...
export default function Header() {
  ...
  return (
    <>
      {/* ==================================================================
          MOBILE & TABLET HEADER - COLLISION FIXED
          ================================================================== */}
      <header className="lg:hidden sticky top-0 z-50 bg-white border-b border-black">
        {/* We use flex-nowrap and justify-between to keep items in their own lanes */}
        <div className="flex items-center justify-between px-3 h-[75px] sm:h-[110px] bg-white relative z-[70]">
          ...
          {/* Center lane: Fluid Logo */}
          {/* Removed 'absolute' - now it pushes away from the buttons */}
          <div className="flex-grow flex justify-center px-2 h-full py-[5%] max-w-[65%] sm:max-w-[75%]">
        ...
        {!isMobileMenuOpen && (
          <div className="bg-white py-2 text-center border-t border-gray-100 relative z-40">
             <span className="text-[11px] sm:text-sm font-bold text-black tracking-wide lowercase first-letter:uppercase">
              {currentDate}
             </span>
          </div>
        )}
      ...
      <header className="hidden lg:block bg-white px-4 xl:px-[30px] pt-1 pb-4">
        ...
        <div className="flex flex-row items-center justify-between my-1">
          <div className="flex flex-col items-start justify-center w-[210px] h-[80px]"> 
          ...
          <div className="flex-1 flex justify-center px-4">
             <div className="relative h-[80px] w-full max-w-[500px]">
              <Image src="/logo.svg" alt="The Polytechnic" fill className="object-contain" priority />
            </div>
          </div>

          <div className="w-[210px] h-[80px] flex items-center justify-end text-[15px] font-bold text-right whitespace-nowrap">
            {currentDate}
          </div>
        </div>

        <nav className="flex flex-wrap items-center justify-center mt-2 gap-8">
          {navItems.map((item) => (
