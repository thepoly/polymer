...
import NewsSectionPage, { type PinnedTile } from '@/components/News/NewsSectionPage';
...
export default async function SectionPageRoute({ params }: Args) {
  ...
  // Fetch news page layout — tile-based grid
  let newsPinnedTiles: PinnedTile[] = [];

  if (section === 'news') {
    type TileShape = { id: string; size: string; articleId: number | null; withImage: boolean };
    type NewsLayoutShape = { tiles?: TileShape[] };
    ...
    try {
      ...
      const doc = layoutResponse.docs[0] as Record<string, unknown> | undefined;
      const layoutJson = doc?.layout as NewsLayoutShape | undefined;

      if (layoutJson?.tiles && layoutJson.tiles.length > 0) {
        const tileArticleIds = layoutJson.tiles
          .map((t) => t.articleId)
          .filter((id): id is number => id !== null);

        if (tileArticleIds.length > 0) {
          const pinnedResponse = await payload.find({
            collection: 'articles',
            where: {
              and: [
                { id: { in: tileArticleIds } },
                { _status: { equals: 'published' } },
              ],
            },
            limit: tileArticleIds.length,
            depth: 1,
            select: {
              title: true, slug: true, subdeck: true, featuredImage: true,
              section: true, kicker: true, publishedDate: true, createdAt: true,
          ...
          newsPinnedTiles = layoutJson.tiles
            .map((tile) => {
              const article = tile.articleId ? pinnedMap.get(tile.articleId) ?? null : null;
              if (!article) return null;
              return {
                article,
                size: tile.size as PinnedTile['size'],
                withImage: tile.withImage,
              };
            })
            .filter((t): t is PinnedTile => t !== null);
        }
      }
    } catch {
      // Table may not exist yet
  ...
  return (
    <main className={`min-h-screen bg-bg-main section-${section} transition-colors duration-300`}>
      ...
      {isOpinion ? (
        ...
        <NewsSectionPage title={sectionTitle} articles={formattedArticles} pinnedTiles={newsPinnedTiles} />
      ) : section === 'features' ? (
        <FeaturesSectionPage
          title={sectionTitle}
          articles={formattedArticles}
