...
export const SenateCard = ({ article }: { article: Article }) => (
    <Link href={getArticleUrl(article)} className="flex flex-col md:flex-row gap-6 cursor-pointer group transition-colors">
        <div className="flex-1 flex flex-col justify-center">
            ...
            <h3 className={`font-copy font-bold text-[24px] md:text-[30px] text-text-main mb-3 leading-[1.06] tracking-[-0.018em] group-hover:text-accent transition-colors ${article.section === "news" ? "!text-[1.2em]" : ""} ${article.section === "features" ? "font-light italic text-[25px] md:text-[31px]" : ""}`}>
                {article.title}
            </h3>
