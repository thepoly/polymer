...
export const ArticleListItem = ({ article }: { article: Article }) => (
  <li className="border-t border-rule py-3 first:border-t-0 first:pt-0">
    <TransitionLink href={getArticleUrl(article)} className="group block" data-header-anchor="text">
      <h4 className={`text-[22px] md:text-[24px] font-bold leading-[1.12] tracking-[-0.01em] text-text-main transition-colors font-copy ${article.section === "opinion" ? "font-light" : ""} ${article.section === "news" ? "!text-[1.2em]" : ""} ${article.section === "sports" ? "font-normal tracking-[0.015em]" : ""} ${article.section === "features" ? "font-light text-[23px] md:text-[25px]" : ""}`}>
        {article.title}
      </h4>
